...
const Card = ({className,id,image,author,title,description,category1,category2,category3}:CardProps) => {
    ...
    return (
        <div className={`${className} ${className?.includes("box")&&"lg:grid lg:grid-cols-2"}`}>
            <div className={`mw-fit mb-6 ${
                className?.includes("image") ? "lg:mb-0" : "lg:mb-8"}`}>
                <img className={`w-full
                ${className?.includes("image")&&"lg:h-full"}`} src={image.source} alt={image.alternative} />
            </div>
            <div>
                ...
                <Link to={`/blog/${id}`}><img src={dark?"/assets/arrowblogwhite.svg":"/assets/arrowblog.svg"} alt="arrow photo" /></Link>
                </div>
